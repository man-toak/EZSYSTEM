...
<html>
  <head> 
    ...
    ?>
      ...
      <?php
      include 'dbconnect.php';
      //$query11 = "SELECT a.*, b.* FROM attendance a, events b WHERE a.event_id=b.id && b.organizer='$organizer'";
      date_default_timezone_set("asia/kuala_lumpur");
      $x = date("Y-m-d");
      $query11 = "SELECT * FROM events WHERE organizer='$organizer' && date>='$x' ORDER BY date";
      $result11 = mysqli_query($conn,$query11) or die('SQL error count down!');
      $row11 = mysqli_fetch_array($result11, MYSQLI_ASSOC);
      if($row11){
         $title = $row11['title'];
         $date = date_create($row11['date']);
         $test = date_format($date,"M d, Y (D)");
         $time = date_create($row11['time']);
         $masaa= date_format($time,"h:i a");
         $end = date_create($row11['end']);
         $habis= date_format($end,"- h:i a");
         $venue = $row11['venue'];
         $xxxxx = $row11['seat'] ;
         $seat = "$xxxxx seat left";
      }
      else{
         $title = "NO EVENT YET";
         $date = "-";
    ...
    <div class="d-flex align-items-stretch">
      ...
      <nav id="sidebar">
        ...
        <ul class="list-unstyled">
          <li class="active"><a href="homepage-participant.php"> <i class="icon-home"></i>Home </a></li>
      ...
      <div class="page-content">
        ...
        <div class="col-lg-7">
          ...
          <div class="col-lg">
            <p>Countdown will appear here if event has registered by organizer</p>
          </div>
          </div>
        </div>
        </section>
            <?php echo '
              <div class="col-lg-4">
                <div class="user-block block text-center">
                    <h3 class="h5">'.$title.'</h3><span>@'.$venue.'</span>
                    <br><div class="contributions">'.$test.'</div>
                    <br><div class="contributions">'.$masaa.' '.$habis.'</div>
                    <br><div class="contributions"><i class="fas fa-chair"></i><strong>'.$seat.'</strong></div>
                </div>
              </div>';
              ?>
        </center>
...
var countdownfunction = setInterval(function() {
  ...
  if (distance < 0) {
    ...
    document.getElementById("demo").innerHTML = "NO EVENT YET";
  }
}, 1000);
</script>
  </body>
