  ...
  <head> 
    ...
    if (isset($_POST['hantar']))
    {
      ...
      $checkout = addslashes($_POST['checkout']);
          ...
          if($row)
          {
            $kah = mysqli_query($conn,"SELECT * FROM attendance WHERE participant_id='" . $participant_id . "' && event_id='" . $event_id . "'");
            $srs = mysqli_fetch_assoc($kah);

            if($srs){
              $attend_status=0;
              $zzz = mysqli_query($conn,"SELECT * FROM attendance WHERE participant_id='" . $participant_id . "' && event_id='" . $event_id . "' && attend_status = '" . $attend_status . "'") ;
              $adoi = mysqli_fetch_assoc($zzz);

                if(!$adoi){

                
                $updatesql = mysqli_query($conn,"UPDATE attendance SET attend_status = '" . $attend_status . "'  WHERE participant_id='" . $participant_id . "' && event_id='" . $event_id . "'");

                if($updatesql){
                  $plus = 1;
                  $updatseat = mysqli_query($conn,"UPDATE events SET seat=seat+'" .   $plus . "'  WHERE id='" .   $event_id . "'");
                  $query = "SELECT * FROM events  where id = '$id'";
                ...
                $message = "Hope to see you again!";}
                  ...
                  $exist = "This user already check-out from this event";
              }
              else
                $error = "This user was not check-in for this event";
          }
          else
      ...
      <div class="page-content">
        <div class="page-header">
          <div class="container-fluid">
            <h2 class="h5 no-margin-bottom">Event Check-Out</h2>
          </div>
        </div>
        ...
        <section class="col-lg-12">
          <div class="block">
            <center><h1><strong id="exampleModalLabel" class="modal-title">CHECK-OUT</strong></h1>
            <div class="modal-body">
              <p>Scan here to complete your attendance.</p>
                ...
                <center><input type="submit" value="Save" class="btn btn-success js-scroll-trigger" name="hantar" hidden></center>
                      ...
                      echo
                        '<div class="alert alert-success alert-dismissible fade show">
                            <strong>Bye2!</strong> ' . $participant_id . "      " . $message . "     ".$nama.'
